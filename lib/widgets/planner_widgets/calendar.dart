...
class _CalendarState extends State<Calendar> {
  ...
  Widget build(BuildContext context) {
    final calendarLayout = getCalendarLayout();
    return Column(
              ...
              children: [
                ...
                Padding(
                  ...
                  child: widget.data.isEmpty ||
                          widget.data[month - 1].plannerData.isEmpty
                      ? Padding(
                          padding: EdgeInsets.all(context.responsiveSize(50)),
                          child: Text("No data available"))
                      : GridView.builder(
                          shrinkWrap: true,
                          physics: const NeverScrollableScrollPhysics(),
                          itemCount: calendarLayout.length,
                          itemBuilder: (context, index) =>
                              calendarLayout[index],
                          gridDelegate:
                              SliverGridDelegateWithFixedCrossAxisCount(
                                  crossAxisSpacing: context.responsiveSize(5),
                                  mainAxisSpacing: context.responsiveSize(5),
                                  crossAxisCount: 7),
                        ),
                )
              ],
  ...
  List<Widget> getCalendarLayout() {
    ...
    print(widget.data[month - 1].plannerData.length);
    try {
      ...
      layout.addAll(widget.data[month - 1].plannerData.map((e) {
        ...
        if (int.parse(e.date) == DateTime.now().day &&
            widget.data[DateTime.now().month].monthIndex == month) {
